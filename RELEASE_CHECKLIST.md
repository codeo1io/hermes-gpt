...
- run the automated connector acceptance gate `python -m pytest -q test_server.py::test_v09_connector_surface_acceptance` (Finance enabled, optional env gates neutralized): it must report the exact expected tool count (137 for the v0.10/v0.11 surface) with all release additions present, no duplicates, and `serverInfo.version` equal to `versioning.VERSION` for this checkout;
