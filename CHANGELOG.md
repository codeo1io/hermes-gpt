...
- Documented the fail-closed profile-aware skill validation that shipped with the rm-027 slice (previously undocumented): plan create/validate and plan readiness reject a declared skill that exists in no resolution root (`skill_not_found`, stage 1), placement scoring hard-rejects an assignee profile that cannot resolve a required skill (`skill_not_resolvable_for_assignee`, stage 2, revalidated on every placement decision and reassignment), and the pure scoring core hard-filters zero-skill targets when skills are required. Canonical module: `operator_skill_resolution.py`.
- Made the skill-name grammar byte-identical between the resolver and the executor: `operator_skill_resolution.validate_skill_name` no longer strips surrounding whitespace before validating (a padded declaration such as `" demo-skill "` is rejected at stage 1 instead of passing validation and failing later at dispatch time), and `operator_skills._validate_skill_name` now uses `fullmatch` like the resolver so a trailing newline can no longer slip through the executor alone. Both validators accept exactly the same names (`^[a-z0-9][a-z0-9._-]*$`, max 64, raw string) — covered by new grammar-parity tests in `test_operator_skill_resolution.py`.
- Adopted upstream `v0.12.0` (commits `8766e2865`, `c3a537693`; 25 files) on top of the fork: the budget D3 hard-block enforcement path (`enforce_budget_breaker` behind `HERMES_GPT_BUDGET_HARD_BLOCK=1` + per-call `confirm` + Operator enabled/direct + per-mission policy flags — all DEFAULT OFF) and the controller L2-rung execution engine (`HERMES_GPT_CONTROLLER_EXECUTE=1` + per-call `confirm` + workspace/direct policy — DEFAULT OFF; `hermes_controller_reconcile` gains a `confirm` parameter, `hermes_controller_status` reports `execution_enabled`). Migrations are guarded additive `ALTER TABLE`s (`controller_telemetry` execution-ledger columns); with both machine gates unset every envelope is byte-identical to v0.11.0 behavior. Fork-local wiring (`operator_skill_resolution` placement/plan gates, DCR advertising gate) verified intact after the three-way merge.
- Documentation truth batch: `docs/oauth.md` now describes the merged dynamic client registration surface (its security model previously claimed "no dynamic client registration"), the `HERMES_GPT_OAUTH_DCR` metadata gate, the `HERMES_GPT_OAUTH_PKCE_MODE` policy knob, and registry persistence/recovery; `docs/operator-mode.md` documents the `HERMES_GPT_OPERATOR_TMPDIR` owner-command scratch directory; the stale `OAuthClient` docstring in `oauth_auth.py` is corrected; `docs/gemini-spark.md`'s intro no longer claims the server advertises no dynamic client registration (it does by default — `HERMES_GPT_OAUTH_DCR=0` is the single-client switch); `docs/oauth.md` also states the per-peer register rate limit and the operator list/purge registry surfaces.
...
- Under MCP SDK 1, tool functions registered as plain `def` through `HermesMCP.add_tool` (`mcp_compat.py`) are now wrapped onto worker threads (`_offload_sync_tool` in `mcp_compat.py`: a `functools.wraps`-preserving coroutine wrapper over `anyio.to_thread.run_sync`), so sync tool bodies execute off the transport's event loop exactly as SDK 2 offloads them — a blocking hermes-gpt tool can no longer stall the SDK 1 lane, and SDK-family drift in sync-tool execution is covered by the new `test_server_sdk1_execution.py` regression suite.
- `hermes_web_extract` and `hermes_vision_analyze` are now native async tools (no `asyncio.run` inside tool bodies on the serving path — the same event-loop-safety goal as the earlier six-tool `asyncio.to_thread` offload, achieved natively); the curated Codex MCP registry bridges them loop-safely from its own worker-thread tool bodies (`asyncio.run` in `build_codex_mcp_server`'s callbacks — safe because both SDK lanes execute those bodies off-loop).
- CI: the PR fast lane now exercises both SDK majors on Python 3.11 (the default `mcp>=2,<3` lane plus `mcp>=1.28.1,<2`), so a pull request catches SDK-1-only execution breakage (e.g. a sync-tool offload regression) before merge.
...
- Added operator registry surfaces (Python-level on `OAuthState`): `list_dynamic_clients()` and `purge_dynamic_client()` (documented in `docs/gemini-spark.md` → “OAuth behavior knobs”). These are library calls today — HTTP/operator-tool wiring is tracked as remaining work under roadmap item rm-055 (orig rm-048 in run-43fe028246d3 numbering).
