...
### DCR registry lifecycle: eviction, import revalidation, and a DCR-gate test
- id: `rm-034` | track: reliability | priority: 78.0 | status: implemented-pending-commit-gate (cycle-3 B1: TTL+eviction+import-parity slice; RFC 7592 management deferred)
- signals: conductor.run-0c897417:assess-F2/F3 + research-R1c, oauth_auth.py:426-433 cleanup() evicts codes/tokens but never dynamic_clients while the registry caps at 64 (oauth_auth.py:371-376) on the unauthenticated /oauth/register surface; state persists via token_store persistence — a permanent resource-exhaustion vector; oauth_auth.py:397-424 import_dynamic_clients skips the _registerable_redirect validation + capacity accounting that live registration enforces (unvalidated persisted state regains grant power on restart); no test covers HERMES_GPT_OAUTH_DCR=0 metadata gating (test_oauth_dynamic_registration.py; only incidental fixture use at test_gemini_compat.py:294)
- acceptance: dynamic clients get a bounded lifecycle — TTL eviction and/or RFC 7592 management (registration_access_token, GET/PUT/DELETE /oauth/register/{client_id}) — proven by a test that fills the registry beyond cap and shows recovery; import_dynamic_clients applies the same redirect validation and capacity accounting as live registration (or rejects loudly with a reason); a dedicated test asserts authorization_metadata advertises registration_endpoint iff HERMES_GPT_OAUTH_DCR is enabled; full suite zero-delta vs the 1497P/28S/0F baseline
- evidence: new tests in test_oauth_dynamic_registration.py green (serial, baseline interpreter); docs/oauth.md lifecycle section updated (rm-036 interlock); diff confined to oauth_auth.py + its tests

### OAuth AS alignment with MCP spec 2026-07-28: RFC 9207 iss + Client ID Metadata Documents
- id: `rm-035` | track: compatibility | priority: 72.0 | status: implemented-pending-commit-gate (slice 1 of 2: RFC 9207 iss complete — success AND error responses + authorization_response_iss_parameter_supported metadata flag, closed by review-fix b4233141; Client ID Metadata Documents design note still open)
- signals: conductor.run-0c897417:research-R1a/R1b, MCP spec 2026-07-28 client-registration.mdx marks RFC 7591 DEPRECATED ("remains available for backwards compatibility") and defines Client ID Metadata Documents as the recommended path (https-scheme URL client_ids, client_id_metadata_document_supported:true AS metadata, server fetches+validates the client's document); changelog minor #7 adds RFC 9207 `iss` parameter (SHOULD) on authorization responses; oauth_auth.py implements only the deprecated RFC 7591 path — grep 2026-09-23 shows zero hits for iss/RFC 9207, client_id_metadata_document, registration_access; metadata documents are stateless and structurally dissolve the rm-034 registry-exhaustion vector (no stored registry at all)
- acceptance: split delivery — (1) RFC 9207 `iss` on authorization responses first (unit test asserting the redirect carries iss; cheap, SHOULD-level, mix-up defense); (2) a recorded design decision on Client ID Metadata Documents (fetch+validate the client's https metadata document at authorize time, advertise the metadata flag, keep RFC 7591 behind HERMES_GPT_OAUTH_DCR for ChatGPT back-compat per the spec's Warning), then implementation if accepted: an integration test registering via metadata document with ZERO registry-slot consumption
- evidence: iss unit test + docs/oauth.md update; design note or implementation with tests; AS is hand-rolled so dual-SDK lanes are unaffected (verify with both lanes green anyway)

### Truthful OAuth docs: rewrite the security model, document the three env knobs, open the CHANGELOG
- id: `rm-036` | track: reliability | priority: 88.0 | status: implemented-pending-commit-gate (cycle-3 B1 U1)
- signals: conductor.run-0c897417:assess-F1/F6, docs/oauth.md:21 claims "no dynamic client registration" — directly contradicted by the merged public RFC 7591 /oauth/register (oauth_auth.py:1319); oauth_auth.py:168 OAuthClient docstring carries the same stale claim; HERMES_GPT_OAUTH_DCR, HERMES_GPT_OAUTH_PKCE_MODE, HERMES_GPT_OPERATOR_TMPDIR appear in ZERO .md files (AGENTS.md documentation rules require exact env names + defaults); CHANGELOG.md has no Unreleased section for any deployed-merge feature
- acceptance: docs/oauth.md describes the DCR endpoint accurately (gating, redirect constraints incl. the import caveat, registry cap, lifecycle); the three env knobs documented with exact names, defaults, and gates in docs/oauth.md / docs/operator-mode.md; stale docstring corrected; CHANGELOG Unreleased entries added covering the deployed-line features (DCR + gating, PKCE mode, operator tmpdir routing, gateway-pid JSON, delegation watermark fix)
- evidence: grep proves zero remaining "no dynamic client registration" claims in docs or docstrings; the three env names greppable in docs; CHANGELOG diff

### Decide the publish.yml runner story (self-hosted vs GitHub-hosted)
- id: `rm-037` | track: reliability | priority: 40.0 | status: candidate (decision item — Git topology is Conductor's call)
...
- id: `rm-038` | track: reliability | priority: 46.0 | status: candidate
...
- id: `rm-039` | track: reliability | priority: 34.0 | status: implemented-pending-commit-gate (partial: unique-tmp+fsync writes at both named sites + 7d janitor; env-mutation-thread-safety and Windows path remain open, and 12 further fixed-.tmp sites across operator_swarm/config/codex/session/token_store/codex_config/operator_runners)
...
- id: `rm-040` | track: maintainability | priority: 16.0 | status: implemented-pending-commit-gate (untracked+ignored with on-disk copy kept; single canonical pid parser in operator_workspace, diagnostics delegates; deploy re-proof is a ci-stage concern)
...
### Structured tool output pilot (outputSchema/structuredContent) — interlock with edd9fb12b6a4 rm-032
- id: `rm-041` | track: compatibility | priority: 30.0 | status: candidate
...
- id: `rm-042` | track: security | priority: 24.0 | status: candidate (design-gated)
