...
- id: `rm-026` | track: reliability | priority: 110.0 | status: implemented (landed at HEAD 4b958379d6 2026-09-22 via the deployed-line merges — version 0.11.0, docs/gemini-spark.md, remediation bundle in-tree; verified 2026-09-23 by run 43fe0282 assess; the fd2bc85f duplicate rm-026 below (same topic, priority 115.0) is merged into this entry)
...
- id: `rm-027` | track: reliability | priority: 55.0 | status: implemented (in-tree at HEAD 4b958379d6: pyproject `pyyaml>=6,<7`, verified 2026-09-23; residual floor gap -> rm-054)
...
- id: `rm-029` | track: reliability | priority: 42.0 | status: implemented (in-tree at HEAD 4b958379d6: updater.py:62,217-227, verified 2026-09-23)
...
- id: `rm-030` | track: reliability | priority: 48.0 | status: implemented (in-tree at HEAD 4b958379d6: ci.yml:28 pins @a86c8ef77170831d7efc7c683542e3b364366168, verified 2026-09-23)
...
- id: `rm-031` | track: maintainability | priority: 12.0 | status: implemented (in-tree at HEAD 4b958379d6: zero dompurify refs in web/package.json, verified 2026-09-23)
...
- id: `rm-026` [DUPLICATE ID — resolved 2026-09-23: identical topic to rm-026 above; merged there; LANDED at HEAD 4b958379d6 via the deployed-line merges; body retained as run-fd2bc85f history] | track: compatibility | priority: 115.0 | status: implemented (merged into rm-026 above)
...
- id: `rm-032` (renumbered 2026-09-23 from the fd2bc85f block's duplicate `rm-027`, resolving the id collision with run 684b9786) | track: reliability | priority: 62.0 | status: implemented (in-tree at HEAD 4b958379d6: pyproject [build-system] setuptools>=77, verified 2026-09-23; originally validated 2026-09-22 via no-isolation prepare_metadata under 77.0.3)
...
- id: `rm-034` (renumbered 2026-09-23 from the fd2bc85f block's duplicate `rm-029`) | track: maintainability | priority: 22.0 | status: implemented (in-tree at HEAD 4b958379d6: `git ls-files -s | awk '$1==160000'` = 0 entries, verified 2026-09-23)
...
<!-- cycle-3 additions below: run 43fe028246d34268a1258c8505e27755 (assess ddaff87ba, research ca1f4abd) — the rm-026..rm-029 duplicate-id collision between runs 684b9786 and fd2bc85f was found by this run's assess and resolved in this edit (fd2bc85f items renumbered rm-032..rm-034; its rm-026 merged into the identical rm-026 above); every status flip below was verified in-tree at HEAD 4b958379d6 on 2026-09-23 -->

### Adopt the self-contained cron parser from origin/master #22 (master CI is red at HEAD)
- id: `rm-047` | track: reliability | priority: 118.0 | status: candidate
- signals: conductor.run-43fe0282:assess-F1, operator_cron.py:981 `from cron.jobs import parse_schedule` (deployed-merge 9a90620d5c) imports a deployment-only package declared nowhere (absent from pyproject/requirements, not vendored); it executes on every hermes_cron_create including dry-run; prod degrades gracefully ({"success": false}) but test_operator_cron.py:643/663/702/720 assert success=true — GitHub-hosted CI run 35683982385 failed exactly there; 5 consecutive red master pushes 03:07-03:39Z 2026-09-22 (9a90620d5c..4b958379d6); the local full-suite green (assess 2026-09-22, /tmp/assess-ddaff87b-full.log, 100% zero FAILED) is an environment mask — the baseline interpreter's venv lives inside the Hermes deployment, which puts `cron` on sys.path
- acceptance: adopt origin/master a93aeb0b1d (PR #22: self-contained croniter parser, +229/-5 operator_cron.py, +39 tests, adds croniter>=2.0,<7 — latest croniter 6.2.4 fits the pin) by content at the merge gate or equivalent; a green master CI run on the adopted commit (verify with `gh run list --repo codeo1io/hermes-gpt` filtered by head_sha); a shipped-module import audit proves no deployment-internal module is imported outside tests
- evidence: `gh run view 35683982385 --log-failed` (test_operator_cron.py:643 CRON_CREATE_ERROR: No module named 'cron'); green run for the adopting sha; grep shows zero undeclared imports in shipped modules

### OAuth dynamic-client registration lifecycle (rate limit + TTL/eviction + RFC 7592 management)
- id: `rm-048` | track: reliability | priority: 112.0 | status: implemented (pending commit gate, run 43fe028246d3 cycle 3, 2026-09-23)
...
- id: `rm-049` | track: correctness | priority: 74.0 | status: implemented (pending commit gate, run 43fe028246d3 cycle 3, 2026-09-23)
...
- evidence: pytest regression green (pre-fix it 500s); documented in the same docs pass as rm-050

### Document the OAuth registration/PKCE env knobs; fix the stale OAuthClient docstring
- id: `rm-050` | track: documentation | priority: 44.0 | status: implemented (pending commit gate, run 43fe028246d3 cycle 3, 2026-09-23)
...
- id: `rm-051` | track: correctness | priority: 26.0 | status: candidate
...
- id: `rm-052` | track: compatibility | priority: 66.0 | status: implemented (pending commit gate, run 43fe028246d3 cycle 3, 2026-09-23)
...
- evidence: pytest asserting iss in both redirect classes + the metadata flag; cheapest win while oauth_auth.py is open for rm-048/rm-049

### Evaluate CIMD as the primary client registration path (strategic)
- id: `rm-053` | track: compatibility | priority: 52.0 | status: deferred (rm-048 landed 2026-09-23 with a TTL-bounded registry that keeps the CIMD slot; revisit with SDK/tooling maturity)
- signals: conductor.run-43fe0282:research-R2, MCP roadmap blog 2026-08-22: "Client ID Metadata Documents (CIMD) as the preferred registration path for clients"; SEP-2468 motivation: "a unique redirect_uri per issuer is not possible with Client ID Metadata Documents (the recommended registration approach) and is operationally expensive with Dynamic Client Registration"; CIMD is stateless — structurally eliminates the rm-048 fill-up class; hermes-gpt ships RFC 7591 DCR only (oauth_auth.py:46, :1209)
- acceptance: a design note decides CIMD adoption timing (SDK/tooling maturity, client support) vs bounded DCR; if adopted: client_id_metadata_documents_supported metadata flag, validation, and tests land; rm-048's hardening keeps a slot for it
- evidence: design note linked here; conformance tests if adopted; docs updated

### Consolidate install manifests to a single source of truth (+ pyyaml floor 6.0.3)
- id: `rm-054` | track: maintainability | priority: 46.0 | status: candidate
- signals: conductor.run-43fe0282:assess-F8 + research-R4/R5, requirements.txt duplicates pyproject deps and DRIFTS: bare `uvicorn` (requirements.txt:2) and bare `pyyaml` (:4) vs the bounded pyproject (uvicorn>=0.30,<1; pyyaml>=6,<7); no workflow references requirements*.txt (grep ci.yml/publish.yml, 2026-09-23); README.md:379 directs operators to `pip install -r requirements-dev.txt`, which lacks ruff while the enforced lint gate needs `[dev]` extras; the pyyaml floor admits 6.0-6.0.2 — sibling upstream PR #75 (codeo1io run 634bf4516c83, open/blocked) claims 6.0.3 fixes CVE-2026-31132; that CVE is UNVERIFIABLE in OSV and the GitHub advisory DB (probed 2026-09-23) and all five repo yaml sites use safe_load — the floor bump is defense-in-depth, not a confirmed-CVE patch
- acceptance: requirements.txt/requirements-dev.txt deleted (or generated from pyproject) with README updated to `pip install -e ".[dev]"`; pyyaml floor raised to >=6.0.3,<7; interlock — reconcile with upstream PR #75 at the catch-up gate (it deletes both files and bounds pyyaml: adopt-by-content or conflict-resolve deliberately)
- evidence: grep shows zero requirements references; README commands verified in a clean venv; pyproject diff; green CI

## Cycle 3 research digest — run 43fe0282 (2026-09-23)

Upstream asimons81/hermes-gpt is FROZEN since 2026-09-21 (7795aa3ce = v0.11.0; fork master 78 ahead / 0 behind) — nothing to catch up; the fork's deployed-line merges already carry v0.11.0. Open upstream items: issue #74 (skill resolution -> rm-028) and PR #75 — from a SIBLING codeo1io fork run (conductor/run-634bf4516c83): deletes requirements*.txt, bounds pyyaml>=6.0.3,<7, drops 12 gitlinks -> rm-054 interlock. MCP: SEP-2468 FINAL (iss in auth responses -> rm-052); CIMD named the preferred registration path over DCR (roadmap blog 2026-08-22 -> rm-053); Tasks extension official (rm-011 signal refreshed); Skills Over MCP WG actively chartering (2026-09-22) -> rm-028 tracks WG output. Python 3.10 EOL 2026-10-31, T-38 days at writing (rm-024). Dependencies: croniter 6.2.4 fits PR #22's >=2,<7 pin; mcp 2.2.0 still latest; the pyyaml CVE claim is unverifiable in public advisory DBs. The ROADMAP duplicate-id defect (rm-026..rm-029 twice) was found by this run's assess (severity medium) and resolved in this edit. **Namespace note:** cycle-3 ids rm-035..rm-046 were already allocated today by sibling run e29c25913c00 (same base HEAD 4b958379d6, its own worktree) — this run's new items therefore occupy rm-047..rm-054; overlap between the two blocks (e.g. their DCR-hardening item ~ rm-048, their OAuth-docs item ~ rm-050) is expected and is deduped at the compound/merge gate, never by renumbering again.
...
- **Deferred to cycle 4+:** rm-053 (CIMD — rm-048 preserved the slot; watch SDK/tooling maturity + the MCP WG), rm-051 (macOS runtime_roots — needs a Darwin lane or containerized verification, unverifiable from this Linux runner), rm-033 (commit the `[tool.ruff]` select alongside CI lane work), rm-024 (3.10 EOL 2026-10-31 — a gate-side matrix decision), rm-054 (manifest consolidation; interlock with upstream PR #75), rm-012 (g4c load-flake class, 4 members), and RFC 7592 read/update/delete now that rm-048's registry is bounded and inspectable.
